<template>
    <div class="main">
        ...
        <div class="main-content">
            ...
            <!-- 详情聊天框 -->
            <detail-component :title="chatTitle" :visible="chatVisible" page_name="consultation" :detailData="detailData"
    ...
    methods: {
        ...
        getListData() {
            this.$Apis.getOrderList({
                ...this.formLine,
                // status: sessionStorage.getItem('userType') == 3 ? "0" : null
            }).then(res => {
                let arr = []
                res.data.map(el => {
                    let typeStr = ''
                    this.typeList.map(cl => {
                        console.log(cl.id == el.type);
                        if (cl.id == el.type) {
                            typeStr = cl.label
                        }
                    })
